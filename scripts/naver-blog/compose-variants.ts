...
export function composeRanking(rows: readonly RankingRow[], asOf: string, siteUrl: string): {
  ...
  const blocks = [
    `한국 주식시장 테마 중 단계별 상위 ${b(`${rows.length}개`)}의 생명주기 점수를 모았습니다. ` +
      `최근 7일 기준 점수가 오른 테마가 ${up(`${risers}개`)}, 내린 테마가 ${down(`${fallers}개`)}, ` +
    ...
    `${asOf} 기준 최근 7일 상승 ${risers}개 / 하락 ${fallers}개입니다. 상위권은 ` +
      ...
      '점수와 관련주는 아래 페이지에서 확인할 수 있습니다. 이 순위는 오늘 기준 활성 테마만 ' +
      '포함하며, 같은 점수라도 오르는 중인지 내리는 중인지에 따라 단계가 달라질 수 있습니다. ' +
      '방법론 페이지에서 산출 과정과 현재 가중치를 확인할 수 있습니다. 수집 주기는 매일이며 주말에도 같은 규칙으로 다시 계산합니다.',
...
export function composeSimilar(
  ...
  const blocks = [
    `${themeName} 관련주 테마가 지금 그리는 점수 곡선과 닮은 과거 사례를 찾았습니다. ` +
    ...
    `정점 일수는 관심이 처음 발생한 날부터 점수가 가장 높았던 날까지의 간격입니다. ` +
      ...
      `총 ${best.pastTotalDays}일이 소요됐습니다. 현재 ${themeName}은 ${best.currentDay}일째를 지나고 있습니다. ` +
      '이 숫자는 진행 상황을 나타내는 관측값이며, 남은 기간을 예측하는 값이 아닙니다.',
    '생명주기 점수는 네이버 검색 관심도, 뉴스 모멘텀, 관련 종목의 활동성과 변동성 ' +
      '네 가지를 가중 합산한 0~100 사이 값입니다. 매일 자동으로 다시 계산되며, ' +
      '수집 범위는 최근 30일입니다. 비교는 통계적 유사성이지 인과관계가 아닙니다.',
    // 관련종목 캡처는 종목이 많으면 2-stocks-a / 2-stocks-b로 쪼개진다(모바일 가독성).
    // 통합·분할 슬롯을 모두 두고 사이에 문단을 끼운다 — 어느 쪽이 매칭되든
    // 이미지가 연속 배치되지 않는다(planBodyActions가 연속 배치를 거부한다).
    slot('2-stocks'),
    slot('2-stocks-a'),
    '개별 테마의 점수만 보면 그 숫자가 높은 편인지 낮은 편인지 판단하기 어렵습니다. ' +
      '과거에 비슷한 곡선을 그린 테마가 어떤 경로를 지났는지 함께 보면 현재 위치를 ' +
      '가늠하는 참고가 됩니다. 두 테마의 재료·참여 종목·시장 환경은 모두 다릅니다.',
    slot('2-stocks-b'),
 ...
 * 기사 1건.
 *
 * `/api/tli/themes/{id}`는 `{ title, link, source, pubDate }`를 준다
 * (app/api/tli/themes/[id]/build-response.ts:147). press/date만 읽던 탓에 뉴스 글의
 * 매체·날짜가 전부 비고 "매체 정보가 표기되지 않았습니다"가 잘못 출력됐다.
 * 양쪽 이름을 모두 받아 계약이 다시 흔들려도 조용히 비지 않게 한다.
 */
...
const pressOf = (n: NewsItem): string | undefined => n.source ?? n.press ?? undefined;
const dateOf = (n: NewsItem): string | undefined => n.pubDate ?? n.date ?? undefined;
...
export function composeNews(
  ...
  const presses = [...new Set(picked.map(pressOf).filter((p): p is string => Boolean(p)))];
  const diff = thisWeek - lastWeek;
  const [y, m] = asOf.split('-');

  const blocks = [
    `${themeName} 관련주 테마의 최근 기사를 모았습니다. 이번 주 관련 기사는 ${b(`${thisWeek}건`)}으로 ` +
      `지난주 ${b(`${lastWeek}건`)} 대비 ${diff >= 0 ? up(`+${diff}건`) : down(`${diff}건`)}입니다. ` +
    ...
    // 통합·분할 슬롯을 모두 두고 사이에 문단을 끼운다 (composeSimilar 주석 참조)
    slot('2-stocks'),
    slot('2-stocks-a'),
    ...
    '기사량이 늘어도 검색 관심도가 따라오지 않으면 점수 상승 폭은 제한적입니다. ' +
      ...
      '테마 단위 집계는 개별 종목의 사정을 담지 못하므로 원문과 공시를 직접 확인하는 것이 정확합니다.',
    '기사 수집은 네이버 뉴스 검색 결과를 기준으로 하며, 테마 키워드와 관련 종목명이 ' +
      '함께 등장하는 기사를 대상으로 합니다. 광고성·홍보성 배포자료로 판단되는 항목은 ' +
      '목록에서 제외했습니다. 제목만으로 내용을 단정하지 않고 원문을 확인합니다.',
    slot('3-trend'),
    ...
    `${asOf} 기준 ${themeName}의 이번 주 기사는 ${thisWeek}건, 지난주는 ${lastWeek}건입니다. ` +
      ...
      `같은 테마라도 기사마다 다루는 종목과 사안이 다르므로 테마 단위 건수만으로 개별 종목을 판단하지 않습니다. ` +
      `위 목록은 어떤 사안이 언제 어느 매체에서 다뤄졌는지를 보여주는 색인이며, ` +
      `종목별 내용은 각 기사와 공시를 직접 확인하는 것이 정확합니다. ` +
      `점수는 기사량 하나가 아니라 검색 관심도·활동성·변동성을 함께 반영합니다. 데이터는 매일 자동으로 다시 집계됩니다.`,
    '이 점수는 네이버 데이터랩 검색 트렌드와 뉴스 건수, KRX 시세를 매일 자동 집계해 계산한 ' +
      '참고용 데이터입니다. 특정 종목의 매수·매도를 권하는 것이 아니며, 투자 판단과 그 결과는 ' +
      '투자자 본인의 책임입니다.',
