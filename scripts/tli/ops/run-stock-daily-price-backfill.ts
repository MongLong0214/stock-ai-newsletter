...
async function main(): Promise<void> {
  ...
  const days = readPositiveInteger(readOption(args, '--days') ?? process.env.TLI_STOCK_BACKFILL_DAYS, 30)
  const callBudget = readPositiveInteger(
    readOption(args, '--call-budget') ?? process.env.TLI_STOCK_BACKFILL_CALL_BUDGET,
    DEFAULT_KIS_DAILY_PRICE_CALL_BUDGET,
  )
  const universe = readUniverse(readOption(args, '--universe') ?? process.env.TLI_STOCK_BACKFILL_UNIVERSE)
  const report = await backfillRecentStockDailyPrices({ endDate, days, callBudget, universe })
