...
export function buildAnalogCandidatesEvidenceStats(input: {
  candidateCount: number
  evidenceCount: number
}): AnalogCandidatesEvidenceStats {
  const expectedArtifacts = Math.max(input.candidateCount, 0)
  const materializedArtifacts = Math.min(Math.max(input.candidateCount, 0), Math.max(input.evidenceCount, 0))
  const dualWriteAttemptCount = expectedArtifacts
  const dualWriteSuccessCount = materializedArtifacts

  return {
    expectedArtifacts,
    materializedArtifacts,
    dualWriteSuccessCount,
    dualWriteAttemptCount,
    auditTrailComplete: expectedArtifacts > 0 && materializedArtifacts === expectedArtifacts,
  }
}
...
async function loadAnalogCandidatesEvidenceStats(): Promise<AnalogCandidatesEvidenceStats> {
  ...
  return buildAnalogCandidatesEvidenceStats({
