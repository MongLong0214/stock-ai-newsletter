 ...
 * Evidence quality is scored against PRD §12 abstention thresholds.
 * No DB calls — these are composable building blocks.
 */
...
export const computeEvidenceQuality = (input: {
  ...
  const t = ABSTENTION_THRESHOLDS

  const supportOk = input.analogSupportCount >= t.minAnalogSupport
  const giniOk = input.candidateConcentrationGini <= t.maxCandidateConcentrationGini
  const weightOk = input.top1AnalogWeight <= t.maxTop1AnalogWeight

  const passCount = [supportOk, giniOk, weightOk].filter(Boolean).length

  // Score: weighted composite
  const supportScore = Math.min(1, input.analogSupportCount / (t.minAnalogSupport * 2))
  ...
  const safeClamped = isFinite(compositeScore) ? Math.round(compositeScore * 100) / 100 : 0
  return { quality, score: safeClamped }
}
