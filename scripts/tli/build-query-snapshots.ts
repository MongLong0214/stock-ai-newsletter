 ...
 *
 * Uses TCAR-001 types (PolicyVersionSet, BoundarySource, ReconstructionStatus).
 * Outputs conform to TCAR-002 bridge schema (QuerySnapshotRow, LabelTableRow).
 */
...
// --- Output Types (match bridge-schema.ts row shapes minus id/timestamps) ---
...
// --- Reconstruction Status Classifier ---
...
export const classifyReconstructionStatus = (input: {
  ...
  // Score is the core signal — without it, reconstruction failed
  if (!input.hasScore) {
    const missing = ['score']
  ...
  // Score present but some features missing → partial
