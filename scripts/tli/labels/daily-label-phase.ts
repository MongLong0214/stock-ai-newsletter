...
const LEGACY_LABEL_HORIZONS = {
  gt_a: GTA_HORIZON_DAYS,
  gt_b: GTB_HORIZON_DAYS,
} as const satisfies Record<PendingLabelType, number>
...
export async function loadPendingBaseDates(input: {
  ...
  for (let from = 0; ; from += PENDING_SCAN_LIMIT) {
    const { data, error } = await supabaseAdmin
      ...
      .eq('horizon_days', LEGACY_LABEL_HORIZONS[input.labelType])
...
export async function runDailyLabelPhase(
  ...
  const isTradingToday = isKoreanTradingDate(today)

  // 비거래일에는 pending 생성을 생략해도, 만기 대상 조회와 재시도는 항상 실행한다.
  if (isTradingToday) {
    try {
  ...
  // 현재 cutoff은 terminal identity를 건너뛰고 pending 또는 누락 identity만 재시도한다.
  try {
    ...
    for (const baseDate of gtATargets) {
      try {
        const isCurrentCutoff = baseDate === finalizeCutoffDate
        const result = await deps.generateGtA({
          baseDate,
        ...
        const terminalCount = result.finalCount + result.censoredCount + result.excludedCount
        if (terminalCount === 0) {
          if (gtAPendingBacklogSet.has(baseDate) || result.totalThemes > 0) {
            warningFailures++
            deps.warn(`GT-A 만기 대상 확정 결과가 0건입니다 (base_date=${baseDate})`)
          }
          continue
        }
        gtAFinalized.push(result)
      } catch (error: unknown) {
        warningFailures++
  ...
  try {
    ...
    for (const baseDate of gtBTargets) {
      try {
        const result = baseDate === finalizeCutoffDate
          ? await deps.generateGtB(baseDate, { missingOrPendingOnly: true })
          : await deps.generateGtB(baseDate, { existingPendingOnly: true })
        if (result.totalThemes > 0) gtBFinalized.push(result)
