...
async function populateKeywords(
  ...
  for (const theme of newThemes) {
    const keywords: Array<{ keyword: string; source: string; isPrimary: boolean }> = []

    // 1) 테마명 자체를 주 키워드로
    keywords.push({ keyword: theme.name, source: 'general', isPrimary: true })
    keywords.push({ keyword: theme.name, source: 'naver', isPrimary: false })
    ...
    try {
      ...
      console.error(`   ⚠️ 종목 수집 실패 (${theme.name}):`, error)
    }
    ...
    try {
      ...
      console.error(`   ⚠️ 자동완성 확장 실패 (${theme.name}):`, error)
    }
