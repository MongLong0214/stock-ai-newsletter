...
function StockList({ stocks }: StockListProps) {
  ...
  return (
    ...
    >
      ...
      {/* 테이블 헤더 */}
      <div className="grid grid-cols-[1fr_auto_auto] gap-3 px-3 py-2 text-[10px] font-mono uppercase tracking-wider text-slate-600 border-b border-slate-700/30 mb-1">
        <span>종목명</span>
        <span>시장</span>
          ...
          {filteredStocks.map((stock, idx) => {
            ...
            return (
              <motion.a
                ...
                className="grid grid-cols-[1fr_auto_auto] gap-3 items-center px-3 py-2.5 rounded-lg border border-transparent hover:bg-white/[0.03] hover:border-emerald-500/20 transition-all duration-200 group cursor-pointer"
              >
                {/* 종목명 + 코드 */}
                ...
                {/* 외부 링크 아이콘 */}
                <ExternalLink className="w-3.5 h-3.5 text-slate-700 group-hover:text-emerald-400 transition-colors" />
              </motion.a>
            )
          })}
        </AnimatePresence>
