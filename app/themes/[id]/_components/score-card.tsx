...
function ScoreCard({ score }: ScoreCardProps) {
  // 가장 높은 기여도를 가진 컴포넌트 찾기
  const dominantComponent = useMemo(() => {
    const weights = { interest: 40, newsMomentum: 25, sentiment: 20, volatility: 15 }
    const labels = {
      interest: '검색 관심',
      newsMomentum: '뉴스 모멘텀',
      sentiment: '기사 논조',
    ...
    const contributions = Object.entries(score.components).map(([key, value]) => ({
      key: key as keyof typeof weights,
      contribution: value * weights[key as keyof typeof weights],
    ...
    >
      ...
      <div className="flex items-start justify-between mb-6">
        ...
        <div className="flex flex-col gap-2">
          <ChangePill
            label="24H"
            value={score.change24h}
            delay={0.4}
          />
          <ChangePill
            label="7D"
            value={score.change7d}
            delay={0.5}
          />
        </div>
      </div>
        ...
        <div>
          ...
          <p className="text-sm font-mono text-slate-200">
            ...
            <span className="text-slate-500 ml-1">
              (기여도 {dominantComponent.contribution.toFixed(1)}pt)
            </span>
          </p>
        </div>
      </motion.div>
...
function ChangePill({ label, value, delay }: { label: string; value: number; delay: number }) {
  ...
  return (
    <motion.div
      ...
      `}
    >
      <span className="text-[10px] font-mono text-slate-500 uppercase tracking-wide min-w-[24px]">
        {label}
      </span>
      {!isZero && (
