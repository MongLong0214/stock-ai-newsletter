    ...
    >
      ...
      <div className="flex gap-6 mt-6 pt-4 border-t border-slate-700/50">
        <div>
          ...
          <div className={`text-lg font-mono font-bold ${score.change24h >= 0 ? 'text-emerald-400' : 'text-red-400'}`}>
            {score.change24h >= 0 ? '+' : ''}{score.change24h.toFixed(1)}
          </div>
        </div>
        <div>
          <span className="text-xs text-slate-500 font-mono">7D</span>
          <div className={`text-lg font-mono font-bold ${score.change7d >= 0 ? 'text-emerald-400' : 'text-red-400'}`}>
            {score.change7d >= 0 ? '+' : ''}{score.change7d.toFixed(1)}
          </div>
        </div>
      </div>
    </motion.div>
