...
<task>
StockMatrix 블로그를 위한 고품질 SEO 키워드 ${count}개를 생성하세요.
...
## 품질 기준
- **키워드는 사람들이 검색창에 실제로 치는 형태여야 한다: 2~3어절.**
  "테마명 + 관련주", "종목명 + 주가", "지표명 + 보는법"처럼 짧고 관용적인 조합.
  네 어절 이상 조합("리비안 관련주 상신이디피 아모그린텍 전망")은 실측 검색량이 0이다 —
  생성 후 네이버 검색광고 API 실측 게이트에서 전부 탈락하므로 만들지 마라.
- 종목명을 2개 이상 이어 붙이지 마라. 종목 비교는 "A vs B" 정확히 두 개까지.
...
(4) reasoning 50자+ (5) 유효 JSON (6) topicArea 필수 (7) 40자 이내·2~3어절의 실검색형 쿼리 (8) 70%+ 테마/종목 기반
</self-validation>
