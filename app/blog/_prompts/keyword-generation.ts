...
- ⚠️ 각 키워드는 반드시 아래 "후킹 트리거" 중 **2개 이상 조합**해야 함 (필수)

## 강력한 후킹 트리거 (Powerful Hooking Triggers)
검색 결과에서 **클릭률을 3배 높이는** 표현 패턴입니다.
반드시 2개 이상 조합하여 사용하세요.

### 1) 손실/실패 회피형 (가장 강력 - 우선 사용)
- 패턴: "X% 손실 방지", "99% 실패하는", "하면 망하는", "절대 하지 말아야", "피해야 할"
- 예시: "초보 투자자 99% 손실 보는 RSI 함정 3가지"

### 2) 구체적 숫자 + 단계형
- 패턴: "3가지", "5단계", "7일 완성", "TOP 10", "단 3분", "3초 판별"
- 예시: "3분 만에 저평가 종목 찾는 PER 체크리스트"

### 3) 질문형/의사결정형
- 패턴: "언제 사야", "얼마에 팔아야", "어떤게 정답", "뭐가 맞을까", "정말 맞나"
- 예시: "RSI 30일 때 매수 vs 40일 때 매수 뭐가 정답일까"

### 4) 타이밍/조건 명확화
- 패턴: "매수 타이밍 잡는", "진입 신호 포착", "정확한 기준", "이 조건이면"
- 예시: "볼린저밴드 하단 터치 후 매수 타이밍 잡는 5초 기준"

### 5) 비교/선택 딜레마
- 패�n: "A vs B 비교", "어느 쪽이 유리", "차이점 총정리", "장단점 비교표"
- 예시: "단타 vs 스윙 손익비 비교 손해 안 보는 선택법"

### 6) 극단/반전형 (강력 후킹)
- 패턴: "절대", "반드시", "100%", "무조건", "아무도 안 알려주는", "숨겨진"
- 예시: "증권사 수수료 아무도 안 알려주는 숨겨진 비용 3가지"

**후킹 조합 공식 (필수):**
```
[손실 회피] + [구체 숫자] = 최강 후킹
예: "99% 손실 보는 RSI 함정 3가지"

[타이밍] + [구체 숫자] = 강력 후킹
예: "매수 타이밍 잡는 5초 체크리스트"

[질문형] + [비교] = 강력 후킹
예: "RSI 30 vs 40 어느 쪽이 유리할까"
```

**정직한 후킹 규칙**:
- 키워드에 포함된 후킹 요소는 본문에서 실제로 "근거/예시/수치/절차"로 해소 가능해야 함
- 과장은 가능하나 거짓은 금지 ("99% 손실" → 실제로는 대부분이라는 의미로 사용 가능)
- 모호한 단정/확정 수익 보장 금지
</analysis-framework>
