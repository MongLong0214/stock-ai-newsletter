...
<korean_seo_guidelines>
  <title_optimization priority="CRITICAL">
    ...
    <bad_examples>
      ❌ "RSI 지표 활용법 총정리 (2024)" → 클릭 욕구 제로
      ❌ "[손실회피] 주식 분석 3가지 방법" → 대괄호 = AI가 쓴 티
      ❌ "주식 초보자를 위한 가이드" → 누구나 쓸 수 있는 제목
      ❌ "이동평균선 사용법 알아보기" → 심리 트리거 없음
      ❌ "PER 낮은 종목 찾는 방법" → 뻔함, 차별화 없음
    </bad_examples>

    <good_examples>
      ✅ "이 신호 나오면 무조건 팔아야 합니다 (손절 타이밍)" → 경고 + 긴급
      ✅ "맨날 고점에 물리는 사람 특징 5가지" → 공감 + 구체성
      ✅ "기관은 알고 개미는 모르는 호가창 비밀" → 대비 + 비밀
      ✅ "RSI 30에서 샀는데 왜 더 떨어질까? 진짜 바닥 찾는 법" → 공감 + 해결
      ✅ "이 종목 사면 안 되는 이유 (차트로 증명)" → 금지 + 증거
      ✅ "3년 전에 알았으면 500만원은 벌었을 매매법" → 후회 + 구체적 금액
    </good_examples>
  </title_optimization>
