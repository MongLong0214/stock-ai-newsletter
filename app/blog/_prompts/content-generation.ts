...
<korean_seo_guidelines>
  <title_optimization priority="CRITICAL">
    ...
    <hooking_formulas>
      **검증된 고CTR 제목 공식:**
      - "[손실회피] + [숫자]" → "90%가 모르는 이동평균선 함정 3가지"
      - "[질문] + [비교]" → "물타기 vs 손절 뭐가 정답일까?"
      - "[비밀] + [숫자]" → "프로만 아는 호가창 분석법 5가지"
      - "[타이밍] + [조건]" → "이 조건이면 무조건 매수해야 하는 이유"
    </hooking_formulas>
    ...
    <good_examples>
      ✅ "90%가 손실 보는 RSI 함정 3가지 (이렇게 피하세요)"
      ✅ "프로 트레이더가 절대 안 쓰는 이동평균선 설정 5가지"
      ✅ "분할매수 3회 vs 5회: 백테스트 결과 공개"
      ✅ "3분 만에 급등주 찾는 거래량 체크리스트"
    </good_examples>
  </title_optimization>
...
<quality_checklist>
  ...
  <seo_requirements>
    [ ] 제목에 "${targetKeyword}" 포함
    [ ] ⚠️ 제목에 후킹 요소 2개 이상 조합 (손실회피/숫자/질문/비밀/비교)
    [ ] ⚠️ 제목이 클릭 욕구를 자극하는가? (밋밋하면 탈락)
    [ ] 첫 100단어 내 타겟 키워드 포함
    [ ] H2 헤딩 중 1-2개에 키워드 포함
    [ ] 본문에 키워드 5-8회 자연스럽게 분포
    [ ] 메타 제목 60자 이내 + 후킹 요소 포함
...
⚠️ **제목(title) 필수 체크:**
- 후킹 요소 2개 이상 조합 필수 (손실회피/숫자/질문/비밀/비교)
- "RSI 활용법" 같은 밋밋한 제목 절대 금지
- 반드시 클릭하고 싶어지는 제목이어야 함

\`\`\`json
{
  "title": "클릭을 부르는 제목 (35-55자, 후킹 2개+ 조합, 예: '90%가 모르는 RSI 함정 3가지')",
