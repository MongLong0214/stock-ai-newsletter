...
describe('script source invariants', () => {
  const prepSource = readFileSync(
    resolve(__dirname, '../../../scripts/prepare-newsletter.ts'),
    'utf-8',
  );
  ...
  it('prepare-newsletter requires service role (no anon fallback)', () => {
    expect(prepSource).toContain('SUPABASE_SERVICE_ROLE_KEY is required');
    expect(prepSource).not.toContain('NEXT_PUBLIC_SUPABASE_ANON_KEY');
  });
  ...
  it('prepare-newsletter uses process.exitCode (not process.exit)', () => {
    expect(prepSource).toContain('process.exitCode');
    expect(prepSource).not.toContain('process.exit(');
  });
  ...
  it('prepare-newsletter stores content and immutable provenance through one RPC receipt', () => {
    expect(prepSource).toContain("rpc('store_newsletter_generation'")
    expect(prepSource).toContain('generationManifest.contentSha256')
    expect(prepSource).toContain('stored.length !== 1')
    expect(prepSource).not.toContain(".from('newsletter_content')\n        .update(")
  });
});
