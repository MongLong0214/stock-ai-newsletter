...
## 3.1 진입가 설정 전략 (상승 추세 분할 매수)
3단계 진입 전략은 상승 추세 확인 후 추가 매수하는 방식입니다:
- **진입 1**: 가장 낮은 가격 (첫 매수 - 지지선 근처)
- **진입 2**: 중간 가격 (추세 확인 후 추가 매수)
- **진입 3**: 가장 높은 가격 (추세 확정 후 최종 매수)

**필수 규칙: entry1 < entry2 < entry3**
...
## 3.2 손절가 설정 전략 (자율 판단)
각 진입가에 대응하는 손절가를 당신의 기술적 분석에 따라 독립적으로 설정하세요.

**필수 규칙 (순서만 지켜주세요):**
- **sl1 < sl2 < sl3** (각 진입가에 비례하여 상승)
- **sl1 < entry1, sl2 < entry2, sl3 < entry3** (각 손절가는 해당 진입가보다 낮음)

### 손절가 설정 시 고려할 기술적 요소들:
- **ATR (Average True Range)**: 변동성 기반 손절 거리 계산
- **주요 지지선**: 피보나치 되돌림, 이전 저점, 이동평균선 하단
- **볼린저밴드 하단**: 통계적 지지선
- **손익비 (Risk:Reward Ratio)**: 목표 수익 대비 손실 비율
- **최근 캔들 패턴**: 스윙 저점, 갭 지지선
- **거래량 프로필**: 고거래량 지지 구간
- **심리적 가격대**: 주요 가격 단위 (예: 70,000원, 180,000원)

**당신의 판단 영역:**
- 변동성이 높은 종목은 손절 폭을 넓게
- 안정적인 종목은 손절 폭을 좁게
- 강력한 지지선이 있다면 그 근처에 손절 설정
- 각 진입가별로 리스크가 다르다면 손절가도 다르게
