...
종목 1:
- ticker: (예: NASDAQ:AAPL)
- name: (예: Apple Inc.)
- close_price: (예: 185.50)

종목 2:
- ticker: (예: NASDAQ:MSFT)
- name: (예: Microsoft Corp.)
- close_price: (예: 378.20)

종목 3:
- ticker: (예: NASDAQ:NVDA)
- name: (예: NVIDIA Corp.)
- close_price: (예: 495.80)
...
종목 1 검증:
✅ "[ticker] NASDAQ listed stock site:nasdaq.com"
✅ "[ticker] stock quote site:finance.yahoo.com"

IF 검색 결과가 없거나 "delisted", "halted", "suspended" 등장:
   ❌ 해당 종목은 사용 불가
   ❌ 즉시 JSON 생성 중단
...
3. 검증 시간대: 장마감(16:00 EST) 이후 데이터만 신뢰

종목 1 전일종가 검증 (5개 소스 필수):

🔍 소스1 (Yahoo Finance - 최우선):
검색어: "[ticker] [전일날짜] close price site:finance.yahoo.com"
결과: $_____ (발견 시각: __:__)

🔍 소스2 (NASDAQ 공식):
검색어: "[ticker] [전일날짜] stock price site:nasdaq.com"
결과: $_____ (발견 시각: __:__)

🔍 소스3 (Google Finance):
검색어: "[ticker] [전일날짜] stock price site:google.com/finance"
결과: $_____ (발견 시각: __:__)

🔍 소스4 (MarketWatch):
검색어: "[ticker] [전일날짜] closing price site:marketwatch.com"
결과: $_____ (발견 시각: __:__)

🔍 소스5 (Investing.com):
검색어: "[ticker] [전일날짜] stock close site:investing.com"
결과: $_____ (발견 시각: __:__)
...
ELSE IF 차이율 ≤ 1.0%:
   ...
   → 검증 기준값(중앙값)으로 대체
   ⚠️ 로그 기록: "[종목명] 전일종가 보정: STAGE4($_) → 기준값($_)"
...
IF 유형 A (소스 부족):
   - 검색어 변경: "[ticker] previous close" → "[ticker] close [date]"
   - 추가 소스 3개 더 시도:
...
IF 유형 B (가격 불일치):
   - 원본 5개 소스 재검색 (캐시 무시)
   - 검색 시간대 확장: [전일 16:00 ~ 23:59 EST] 데이터
   - 소수점 포함 가격도 허용 (반올림 처리)
   - 차이율 재계산 후 판정

IF 유형 C (날짜 오류):
   - 전일 영업일 재계산 (미국 주식시장 휴장일 체크)
   - 최근 5일 영업일 모두 확인
   - 가장 최근 거래일 데이터 사용
...
"[STEP 3] 종목1 전일종가 검증 중...
 ...
 → 소스4: ✅ $185.45
 → 소스5: ✅ $185.50
 → 결과: 성공 (중앙값 $185.50, STAGE4 일치율 100%)"
