...
핵심_원칙:
  ...
  - 오늘(또는 가장 최근 거래일) 기준 값만 허용하며 과거 날짜 값은 사용 금지한다.
...
각 종목에 대해:
  ...
  검색쿼리_1 = 쿼리생성({
    ...
    날짜: "[target_date]",  // STEP 0에서 계산된 실제 거래일 사용
    사이트필터: "finance.naver.com OR stockplus.com"
  })

  // 예시: "삼성전자 005930 RSI ADX ATR 전일 종가 [target_date] site:finance.naver.com OR site:stockplus.com"
...
각 종목에 대해:
  ...
  추출패턴의 각 [지표명, 사양]에 대해:
    ...
    while ((매칭 = 패턴.exec(종목.검색결과텍스트)) !== null):
      ...
      // 날짜 필터: 오늘/최근 거래일 값만 허용, 날짜 불명확 시 폐기
      만약 매칭.input에 [target_date] 또는 최신 거래일 표기가 없으면:
        계속
  ...
  // 전일 종가 동기화: 실제 검색 결과 기준으로 close_price 갱신
  만약 종목.실제값["전일 종가"] !== undefined:
    종목.close_price = 종목.실제값["전일 종가"]
...
각 종목에 대해:
  ...
  만약 검증된지표.length < 10:
    ...
    만약 종목.실제값["전일 종가"] !== undefined:
      추가지표후보.push(\`전일 종가 \${종목.실제값["전일 종가"]}원\`)
...
만약 실패_종목수 >= 1:
  ...
  throw Error("Stage 6 검증 실패: 현재 데이터 확보 불가")
