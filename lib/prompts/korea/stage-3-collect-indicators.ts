...
❌ 유사 지표로 대체 금지 (예: Williams %R 실패 시 Stochastic 값으로 채우는 것 금지)
✅ 기본 공식으로 직접 계산 시도
...
각 지표당 쿼리 변형을 순차 시도하되, 성공하면 즉시 중단:

| 결과 | 행동 |
|------|------|
| 2개 이상 소스 일치 | 검색값 사용 (신뢰도 HIGH) |
| 1개 소스만 성공 | 검색값 사용 + 출처 명시 (신뢰도 HIGH, 단일 소스 표기) |
| 검색 실패, 원시 데이터 확보됨 | 계산 폴백 (신뢰도 MEDIUM, 입력 데이터 검증 필수) |
| 검색 실패, 원시 데이터도 없음 | 해당 지표 제외 (환각 금지) |

"계산 폴백"을 사용하려면:
→ 계산에 필요한 원시 데이터(종가, 거래량 등)가 검색으로 확보되어야 함
→ 원시 데이터 없이 "RSI를 추정"하는 것은 환각임

⚠️ 모든 지표를 채우는 것보다 확보된 지표의 정확성이 우선입니다.
⚠️ 검색 예산은 유한합니다. 실패한 지표는 미련 없이 제외하고 다음으로 진행하세요.
...
1단계: 공식 금융 사이트 검색 (naver/daum/krx, 쿼리 변형 시도)
2단계: 글로벌 사이트 검색 (TradingView, Investing, StockCharts)
3단계: 과거 데이터 수집 시도 (계산에 필요한 일수만큼)
4단계: 직접 계산 (명시된 공식 사용, 입력 데이터 검증 필수)
5단계: 실제 데이터 확보 실패 시 해당 지표 제외
6단계: 종목 교체 (최최후 — TIER 1 지표 과반 실패 시)
...
- 30개 지표 중 15-19개 = 최소 (TIER 1 우선 재검색 후 진행)
- 30개 지표 중 15개 미만 = 해당 종목 수집 부실 표기 (STAGE 4에서 순위 하향)

우선순위:
- TIER 1 (01~10번)은 최우선 — 전 종목에서 최대한 확보
- TIER 2/3는 확보 가능한 범위에서 수집, 실패 시 제외하고 진행
- 30개 종목 전체에 대해 수집을 완료할 때까지 진행 (중도 포기 금지)
...
function getIndicatorsDefinition(): string {
  return `━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
【TIER 1: 핵심 10개 지표 - 아래 쿼리 변형을 순차 시도, 성공 즉시 중단】
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
...
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
【TIER 2: 중요 10개 지표 - 쿼리 변형 순차 시도 (성공 즉시 중단)】
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━

공통 검색 패턴 (지표명만 바꿔 순차 시도):
  ① "[종목명] [지표명] site:tradingview.com"
  ② "[종목명] [지표명] site:investing.com"
  ③ "[종목명] [지표 한글명] site:finance.naver.com"

11. Williams %R (14일)
    계산 폴백: %R = (14일최고가 - 전일 종가) / (14일최고가 - 14일최저가) × -100
    점수 기준:
    ✅ -50 < %R ≤ -20 (상승 모멘텀) = 90
    ✅ -80 < %R ≤ -50 (중립·반등 초입) = 75
    ⚠️ %R > -20 (과매수) = 40
...
12. ROC (Rate of Change, 12일)
    계산 폴백: ROC = (전일 종가 - 12일전 종가) / 12일전 종가 × 100
    점수 기준:
    ✅ 0 < ROC ≤ 8% (건전한 상승) = 90
    ✅ 8 < ROC ≤ 15% (강한 상승, 과열 접근) = 70
    ⚬ -3 ≤ ROC ≤ 0 (바닥 다지기) = 60
    ⚠️ ROC > 15% (과열) = 45
    ❌ ROC < -3% (하락 추세) = 35

13. CCI (Commodity Channel Index, 20일)
    계산 폴백: CCI = (TP - TP의 20일 SMA) / (0.015 × 평균편차), TP = (고+저+종)/3
    점수 기준:
    ✅ 0 ≤ CCI ≤ 100 (상승 초입) = 90
    ✅ -100 ≤ CCI < 0 (반등 대기) = 70
    ⚠️ CCI > 100 (과매수 — STAGE 4 제외 대상) = 45
    ❌ CCI < -100 (하락 추세) = 40

14. MFI (Money Flow Index, 14일)
    계산 폴백: RSI 공식에 거래대금 가중 (Typical Price × Volume)
    점수 기준:
    ✅ 50 < MFI ≤ 70 (자금 유입 강세) = 90
    ✅ 40 ≤ MFI ≤ 50 (중립) = 75
    ⚬ 70 < MFI ≤ 80 (과열 접근) = 60
    ⚬ MFI < 40 (약세/과매도) = 55
    ⚠️ MFI > 80 (자금흐름 과열 — STAGE 4 제외 대상) = 40

15. CMF (Chaikin Money Flow, 20일)
    계산 폴백: CMF = Σ(MFV) / Σ(Volume), MFV = ((종-저)-(고-종))/(고-저) × Volume
    점수 기준:
    ✅ CMF > 0.1 (뚜렷한 매집) = 90
    ✅ 0 < CMF ≤ 0.1 (약한 매집) = 75
    ⚬ -0.05 ≤ CMF ≤ 0 (중립) = 55
    ❌ CMF < -0.05 (분산/매도 우위) = 35

16. Parabolic SAR
    계산 폴백: 표준 파라미터 (AF 0.02, 최대 0.2), 과거 고저가 필요
    점수 기준:
    ✅ 매수 전환 직후 1~3일 (SAR가 가격 아래로 이동) = 100
    ✅ SAR < 전일 종가 (상승 지속) = 85
    ⚬ 전환 임박 (SAR와 가격 근접) = 60
    ❌ SAR > 전일 종가 (하락 신호) = 35

17. Ichimoku (일목균형표)
    계산 폴백: 전환선(9), 기준선(26), 선행스팬A/B — 과거 52일 고저가 필요
    점수 기준:
    ✅ 종가>구름대 AND 전환선>기준선 AND 구름 상승 (삼역호전) = 100
    ✅ 종가 > 구름대 상단 = 85
    ⚬ 종가가 구름대 내부 = 60
    ❌ 종가 < 구름대 하단 = 35

18. SuperTrend (10, 3)
    계산 폴백: ATR 기반 밴드 — (고+저)/2 ± 3×ATR(10)
    점수 기준:
    ✅ 매수 전환 직후 1~3일 = 100
    ✅ 매수 신호 유지 중 = 85
    ❌ 매도 신호 = 35
...
19. VWAP (Volume Weighted Average Price)
    계산 폴백: Σ(가격×거래량) / Σ(거래량) — 전일 일중 데이터 필요 (미확보 시 제외)
    점수 기준:
    ✅ 종가가 VWAP 0~+3% (지지 확인) = 90
    ✅ 종가 ≈ VWAP ±1% (균형) = 75
    ⚬ 종가가 VWAP +3~7% (이격 확대) = 60
    ⚠️ 종가가 VWAP +7% 초과 (과열) = 40
    ❌ 종가 < VWAP -1% (매도 우위) = 45

20. 52주 고저점
    계산 폴백: 위치% = (전일 종가 - 52주저가) / (52주고가 - 52주저가) × 100
    점수 기준:
    ✅ 위치 60~85% (고점 접근, 돌파 여력) = 90
    ✅ 위치 40~60% (중간대 상승 여력) = 75
    ⚬ 위치 85~95% (신고가 임박) = 60
    ⚬ 위치 15~40% (바닥 탈출 시도) = 55
    ❌ 위치 <15% (바닥권) 또는 >95% (신고가 추격 리스크) = 45

━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
【TIER 3: 고급 10개 지표 - 쿼리 변형 순차 시도 (성공 즉시 중단)】
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━

공통 검색 패턴: TIER 2와 동일 (tradingview → investing → naver 순)
⚠️ TIER 3는 검색 성공률이 낮은 지표군 — 2회 시도 실패 시 즉시 계산 폴백 또는 제외

21. Keltner Channel (20일, EMA ± 2×ATR)
    점수 기준 (밴드 내 위치%):
    ✅ 위치 40~75% (중단~상단 사이 상승) = 90
    ⚬ 위치 25~40% (하단 반등) = 70
    ⚠️ 위치 > 75% (상단 돌파 과열) = 50
    ❌ 위치 < 25% (하단 이탈 약세) = 45

22. Donchian Channel (20일 고저 채널)
    점수 기준:
    ✅ 상단 돌파 직전 (상위 70~95%) = 85
    ✅ 20일 신고가 갱신 직후 = 70
    ⚬ 중간부 = 60
    ❌ 하단권 (하위 30%) = 40

23. Aroon (25일)
    계산 폴백: AroonUp = (25 - 고점 이후 경과일)/25 × 100, Down은 저점 기준
    점수 기준:
    ✅ AroonUp > 70 AND AroonDown < 30 (강한 상승) = 95
    ✅ AroonUp > AroonDown (상승 우위) = 75
    ⚬ 둘 다 중간 (수렴) = 55
    ❌ AroonDown > 70 (하락 우위) = 30

24. Elder Ray (13일)
    계산 폴백: Bull Power = 고가 - EMA13, Bear Power = 저가 - EMA13
    점수 기준:
    ✅ Bull > 0 상승 AND Bear < 0 축소 (매수 우위 강화) = 90
    ✅ Bull > 0 (매수 우위) = 75
    ⚬ 혼조 = 55
    ❌ Bull < 0 AND Bear 확대 (매도 우위) = 35

25. Force Index (13일)
    계산 폴백: FI = (금일종가 - 전일종가) × 거래량의 13일 EMA
    점수 기준:
    ✅ FI > 0 상승 + 거래량 동반 = 90
    ✅ FI > 0 = 75
    ⚬ 0 부근 횡보 = 55
...
26. Ease of Movement (EMV, 14일)
    계산 폴백: EMV = 중간가 변동 / (거래량/고저폭)의 14일 평균
    점수 기준:
    ✅ EMV > 0 (적은 거래량으로 상승 용이) = 85
    ⚬ 0 부근 = 55
    ❌ EMV < 0 (상승 저항) = 40
...
27. Accumulation/Distribution (A/D Line)
    계산 폴백: CMF의 MFV 누적합
    점수 기준:
    ✅ A/D 상승 + 가격 상승 (동반 상승) = 90
    ✅ A/D 상승 + 가격 횡보 (매집 진행) = 80
    ⚬ 횡보 = 55
    ⚠️ A/D 하락 + 가격 상승 (부정 다이버전스) = 40
    ❌ 동반 하락 = 30

28. Know Sure Thing (KST)
    계산 폴백: 4개 기간(10,15,20,30) ROC 가중합의 9일 Signal 교차
    점수 기준:
    ✅ KST > Signal AND KST > 0 = 90
    ✅ KST > Signal (골든크로스) = 75
    ⚬ 0 부근 수렴 = 55
    ❌ KST < Signal < 0 = 30

29. Vortex Indicator (VI, 14일)
    계산 폴백: VI+ = Σ|금일고가-전일저가| / ΣTR, VI- = Σ|금일저가-전일고가| / ΣTR
    점수 기준:
    ✅ VI+ > VI- 교차 직후 = 95
    ✅ VI+ > VI- 유지 = 80
    ⚬ 수렴 = 55
...
30. Chaikin Oscillator (3일 EMA - 10일 EMA of A/D)
    점수 기준:
    ✅ 0 상향 돌파 직후 = 90
    ✅ CO > 0 상승 = 85
    ⚬ 0 부근 = 55
