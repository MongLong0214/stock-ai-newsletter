...
import { STOCK_ANALYSIS_PROMPT_NASDAQ } from '../prompts/stock-analysis-prompt-nasdaq';
...
/**
 * Gemini API 호출 (NASDAQ 전용)
 */
async function callGeminiAPI(genAI: GoogleGenAI): Promise<string> {
  console.log('[API 호출 시작 - NASDAQ] generateContent 요청 중...');
  ...
  try {
    response = await withTimeout(
      genAI.models.generateContent({
        ...
        contents: [
          {
            role: 'user',
            parts: [{ text: STOCK_ANALYSIS_PROMPT_NASDAQ }],
          },
        ],
        config: {
    ...
    console.log('[API 호출 성공 - NASDAQ] 응답 수신 완료');
  } catch (error) {
    console.error('[API 호출 실패 - NASDAQ]', error);
    throw error;
  }
...
/**
 * Gemini NASDAQ 주식 추천 분석 실행 (Vertex AI)
 */
export async function getGeminiRecommendationNasdaq(): Promise<string> {
  // 환경 변수 검증
  if (!process.env.GOOGLE_CLOUD_PROJECT) {
    return '⚠️ GOOGLE_CLOUD_PROJECT 환경 변수가 설정되지 않았습니다.';
  }

  console.log(`[Gemini NASDAQ] Using Vertex AI (Project: ${process.env.GOOGLE_CLOUD_PROJECT})`);

  if (geminiBreaker.isOpen()) {
    console.warn('[Gemini NASDAQ] Circuit breaker open');
    return '⚠️ Gemini 서비스가 일시적으로 불안정합니다.';
  }

  try {
    // GoogleGenAI 초기화 (Vertex AI)
    console.log(`[Gemini NASDAQ] Vertex AI Location: ${VERTEX_AI_LOCATION}`);
    ...
    for (let attempt = 1; attempt <= MAX_RETRY; attempt++) {
      ...
      console.log(`[Gemini NASDAQ] 시도 ${attempt}/${MAX_RETRY}`);
      ...
      try {
        ...
        // 응답 로깅
        console.log(`\n━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━`);
        console.log(`📥 [Gemini NASDAQ 원본 응답] (${attempt}/${MAX_RETRY})`);
        console.log(`━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━`);
        console.log(result);
        console.log(`━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━\n`);
        ...
        if (validJSON) {
          console.log(`✅ [Gemini NASDAQ] 유효한 JSON 응답 받음 (${attempt}/${MAX_RETRY})`);
          if (validJSON !== result) {
        ...
        // 재시도 처리
        console.warn(`⚠️ [Gemini NASDAQ] JSON 검증 실패 (${attempt}/${MAX_RETRY})`);

        if (attempt < MAX_RETRY) {
          console.log(`🔄 [Gemini NASDAQ] ${retryDelay / 1000}초 후 재시도... (Exponential Backoff)`);
        ...
        console.warn(`⚠️ [Gemini NASDAQ] API 오류 (${attempt}/${MAX_RETRY}): ${errorMsg}`);
    ...
    console.error('❌ [Gemini NASDAQ Error]', error);
