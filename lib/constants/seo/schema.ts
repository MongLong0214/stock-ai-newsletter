...
export const schemaConfig: SchemaConfig = {
  ...
  serviceDesc: `한국 주식 투자자를 위한 무료 AI 주식 분석 뉴스레터 서비스입니다. AI가 RSI(상대강도지수), MACD(이동평균수렴확산), 볼린저밴드, 이동평균선 등 ${indicatorCount}개 기술적 지표로 분석한 ${markets} ${stockCount}개 종목을 매일 ${deliveryTime} 무료 이메일로 발송합니다. 웹사이트에서는 ${siteConfig.themeCountFloor}+ 테마 생명주기 데이터를 추가로 제공합니다. 투자 권유나 매매 추천이 아닌 참고용 기술적 분석 정보만 제공합니다.`,
} as const;
