      ...
      try {
        if (IS_SIX_DIGIT.test(query.trim())) {
          ...
          if (!themeData && (!searchData || (Array.isArray(searchData) && searchData.length === 0))) {
            return {
              content: [{ type: 'text' as const, text: formatEmptyResult(CONTEXT, `No stock found for code "${query}". Verify the 6-digit Korean stock code.`) }],
            };
          }

          const combined = { stockThemes: themeData, searchResults: searchData };
          return {
            content: [{ type: 'text' as const, text: formatResult(combined, CONTEXT) }],
          };
        }
        ...
        if (Array.isArray(data) && data.length === 0) {
          return {
            content: [{ type: 'text' as const, text: formatEmptyResult(CONTEXT, `No stocks found for "${query}". Try a different company name or check the spelling.`) }],
          };
        }
