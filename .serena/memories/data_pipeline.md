...
5. 비교 분석 → calculate-comparisons.ts → run-comparisons.ts
   ├─ 현재 테마 vs 과거 테마 (Decay/Dormant) 비교
   ├─ compositeScore() = 0.5*curveSim + 0.3*featureSim + 0.2*keywordSim
...
6. 006_add_comparison_pillar_scores.sql — theme_comparisons에 3-pillar 컬럼
