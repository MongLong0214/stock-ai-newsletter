...
3. 네이버 금융 종목 수집 → collectors/naver-finance-themes.ts
   ...
   └─ upsertThemeStocks() → theme_stocks 테이블
...
| lib/tli/__tests__/calculator.test.ts | calculateLifecycleScore | 9 |
...
| lib/tli/__tests__/prediction.test.ts | calculatePrediction | 10 |
...
## 예측 로직 (lib/tli/prediction.ts) — 2026-02-10 추가
...
| outcome_verified | BOOLEAN | 결과 검증 여부 (2026-02-10 추가) |
| trajectory_correlation | REAL | 궤적 상관계수 (2026-02-10 추가) |
| stage_match | BOOLEAN | 단계 일치 여부 (2026-02-10 추가) |
| verified_at | DATE | 검증일 (2026-02-10 추가) |
| UNIQUE(current_theme_id, past_theme_id, calculated_at) | | |

### prediction_snapshots (2026-02-10 신규)
| 컬럼 | 타입 | 설명 |
|------|------|------|
| id | UUID PK | |
| theme_id | UUID FK → themes | |
...
### comparison_calibration (2026-02-10 신규)
| 컬럼 | 타입 | 설명 |
|------|------|------|
| id | UUID PK | |
...
7. 007_create_prediction_snapshots.sql — prediction_snapshots 테이블 (2026-02-10)
8. 008_add_comparison_outcome_tracking.sql — theme_comparisons 검증 컬럼 + comparison_calibration (2026-02-10)
