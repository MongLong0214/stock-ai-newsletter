...
5. 비교 분석 → calculate-comparisons.ts (197 lines)
...
### 3-Pillar 유사도 (v2 redesign)
| Pillar | 가중치 | 방법 |
|--------|--------|------|
| Feature Similarity | 동적 (max 0.65) | 코사인 유사도 (7D: interest, news, sentiment, activeDays, newsTotal, priceChange, volume) |
| Curve Similarity | 동적 | RMSE shape (60%) + derivative Pearson (40%) |
| Keyword Similarity | 동적 | Jaccard 계수 (theme_keywords 집합 비교) |

- exp scaling: `exp(-distance * 0.7)` for feature similarity
- 최소 요건: 현재 테마 >= 7일 interest, 과거 테마 >= 14일
- 임계값: 0.40 이상만 유의미한 비교로 저장
- Weight cap: 0.65 (단일 pillar 지배 방지)
...
- 네이버 API가 `&quot;` 포함 제목 반환 → DB에 그대로 저장 → UI에 `&quot;` 노출
