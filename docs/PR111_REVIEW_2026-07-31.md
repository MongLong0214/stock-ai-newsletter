...
리뷰 중 제기했다가 빌드 산출물로 확인해 **오탐으로 철회한 항목이 1건 있다(H-5).** 아래에
철회 사유와 근거를 남겨 둔다.
...
### ~~H-5. OG 배경 이미지 런타임 로드가 Vercel에서 실패할 수 있음~~ — 오탐

`lib/og-template.tsx`가 `readFileSync(join(process.cwd(), 'public', 'og-background-v1.png'))`로
런타임에 파일을 읽고 `next.config.ts`에 `outputFileTracingIncludes`가 없어 서버리스 번들에
포함되지 않을 것으로 판단했으나, **빌드 산출물로 확인한 결과 오탐이다.**

Next.js 의존성 트레이서가 이 `readFileSync` 패턴을 정적 분석해 자동으로 포함한다.
`outputFileTracingIncludes` 없이 빌드했을 때 OG 이미지 라우트 15개 전부가 해당 PNG를
트레이싱한다.

```
$ find .next/server/app -name "route.js.nft.json" -path "*image*" | xargs grep -l og-background-v1.png | wc -l
15   # 전체 15개
```
